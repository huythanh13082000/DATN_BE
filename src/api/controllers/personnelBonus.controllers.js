...
const updatePersonnelBonus = async (req, res) => {
  ...
  try {
    await personnelBonusModel.findByIdAndUpdate({ _id }, { ...data, updateAt: Date.now() })
    const personnelBonus = await personnelBonusModel.findOne({ _id })
...
const deletePersonnelBonus = async (req, res) => {
  const _id = req.params.id
  try {
    await personnelBonusModel.findByIdAndDelete({ _id })
